...
for i in range(len(emu.redshifts)):
    nn_models.append(keras.models.load_model(f'./NN_MODELS/COLA_EMU_1_NHID=2_NEURONS=1024_z={emu.redshifts[i]:.3f}'))
...
def get_boost(cosmo_params, ks = emu.ks, z = emu.redshifts):
    ...
    boost_in_cola_ks_zs = np.zeros((len(emu.redshifts), len(emu.ks)))
