...
#------------------------------------------------------------------------------------------------------------
def pca_reconstruction_analysis(num_pcs, norm_log_boosts, log_boosts, sample, boosts, save=False, imgName='pc_analysis.pdf'):
    '''
    Plots the reconstruction errors for a given number of PCs
    The boosts can be replaced by Qs
    '''
    pcas = []
    reduced_norm_log_boosts = np.zeros((len(sample), len(redshifts), num_pcs))
    reconstructed_boosts = np.zeros((len(sample), len(redshifts), len(norm_log_boosts[0,0,:])))
    for i in range(len(redshifts)):
        pca = PCA(n_components=num_pcs)
        reduced_norm_log_boosts_thisz = pca.fit_transform(norm_log_boosts[:,i,:])
        reconstructed_norm_log_boosts_thisz = pca.inverse_transform(reduced_norm_log_boosts_thisz)
        pcas.append(pca)
        reduced_norm_log_boosts[:,i,:] = reduced_norm_log_boosts_thisz
        reconstructed_log_boosts_thisz = unnormalize_array(reconstructed_norm_log_boosts_thisz, log_boosts)
        reconstructed_boosts_thisz = np.exp(reconstructed_log_boosts_thisz)
        reconstructed_boosts[:,i,:] = reconstructed_boosts_thisz
    # Plot reconstructed data
    for i, point in enumerate(sample):
        for j, redshift in enumerate(redshifts):
            plt.semilogx(ks, reconstructed_boosts[i,j,:]/boosts[i,j,:] - 1, c='gray',alpha=0.5)
